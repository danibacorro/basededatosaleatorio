...
FOR r IN c_errores LOOP
    ...
    CASE r.RETURNCODE
        WHEN 00988 THEN DBMS_OUTPUT.PUT_LINE(CHR(9)||'- Falta la contraseña o no es válida');
        WHEN 01004 THEN DBMS_OUTPUT.PUT_LINE(CHR(9)||'- Usuario no suministrado/Inicio de sesión automático fallido');
        WHEN 01005 THEN DBMS_OUTPUT.PUT_LINE(CHR(9)||'- Contraseña no suministrada');
        ...
        WHEN 01045 THEN DBMS_OUTPUT.PUT_LINE(CHR(9)||'- Usuario sin privilegio para inciar sesión (CREATE SESSION)');
        ...
        WHEN 28002 THEN DBMS_OUTPUT.PUT_LINE(CHR(9)||'- Advertencia de caducidad cercana de contraseña');
        WHEN 28009 THEN DBMS_OUTPUT.PUT_LINE(CHR(9)||'- La conexión a sys se debe realizar desde sysdba o sysoper');
        WHEN 28011 THEN DBMS_OUTPUT.PUT_LINE(CHR(9)||'- Advertencia de caducidad cercana de contraseña');
